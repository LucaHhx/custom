  ...
  if (res.code === 0) {
    ...
    res.data.columns &&
          res.data.columns.forEach(item => {
            ...
            if (intTypes.some(intfd => intfd === item.dataType)) {
              if (item.dataType === 'tinyint') {
                item.dataTypeLong = '8'
              } else if (item.dataType === 'smallint') {
                item.dataTypeLong =   '16'
              } else if (item.dataType === 'bigint') {
                item.dataTypeLong =  '64'
              }else{
                item.dataTypeLong =  '32'
              }
            }
...
const setFdMap = async() => {
  const fdTypes = ['string', 'int', 'bool', 'float64', 'time.Time','richtext']
