<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        {{- else }}
        <el-table-column {{- if .Sort}} sortable{{- end}} align="left" label="{{.FieldDesc}}" show-overflow-tooltip prop="{{.FieldJson}}" width="120" />
        {{- end }}
        {{- end }}
      ...
      <el-scrollbar height="500px">
        ...
        {{- range .Fields}}
          ...
          {{- if eq .FieldType "richtext" }}
             <el-input v-model="formData.{{.FieldJson}}" :clearable="{{.Clearable}}"   placeholder="请输入{{.FieldDesc}}"  type="textarea"  :autosize="{ minRows: 10, maxRows: 20 }" />
          {{- end }}
          {{- if eq .FieldType "int" }}
          {{- if .DictType}}
...
{{- range .Fields}}
{{- if eq .FieldType "enum"}}
const {{.FieldJson}}Opts = { 1: 'Normal', 2: 'Close'}
{{- end}}
{{- end }}
