...
import (
	...
	ast2 "custom-server/utils/ast"

	"custom-server/resource/autocode_template/subcontract"
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		...
		if strings.Contains(fileSlice[n-1], "js") {
			...
			data.autoMoveFilePath = filepath.Join(global.GVA_CONFIG.AutoCode.Root,
				global.GVA_CONFIG.AutoCode.Web, global.GVA_CONFIG.AutoCode.WForm, fmt.Sprintf("/%s", data.autoPackage), filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.GVA_CONFIG.AutoCode.Root,
				global.GVA_CONFIG.AutoCode.Web, global.GVA_CONFIG.AutoCode.WTable, fmt.Sprintf("/%s", data.autoPackage), filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
